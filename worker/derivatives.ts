...
export const DERIVATIVE_VERSION = 'bybit-usdt-perpetual-v4';
export const derivativeKind = (metric: DerivativeMetric) =>
  metric.replace('_daily', '') as 'funding' | 'open_interest' | 'long_account_ratio';
...
export function parseDerivativeRows(
  ...
  for (const candidate of rows) {
    ...
    if (
      ...
      (kind === 'open_interest' && value < 0) ||
      (kind === 'funding' && Math.abs(value) > 1) ||
      (kind === 'long_account_ratio' &&
        (value < 0 ||
          value > 1 ||
          typeof row.sellRatio !== 'string' ||
          !Number.isFinite(Number(row.sellRatio)) ||
          Number(row.sellRatio) < 0 ||
          Number(row.sellRatio) > 1 ||
...
export async function updateDerivatives(
  ...
  if (kind === 'long_account_ratio') params.set('period', daily ? '1d' : '1h');
...
export async function readDerivativeSeries(
  ...
  const kind = derivativeKind(metric);
  const daily = metric.endsWith('_daily');
  const [rows, extent, state, cursor] = await Promise.all([
    db
      .prepare(
    ...
    readState<number | null>(db, 'cursor:' + key(asset, metric), null),
  ]);
  const data = rows.results.slice(0, limit);
  const age = daily ? 3 * DAY : metric === 'funding' ? 36 * 3600 : 3 * 3600;
  return {
    data,
    price: [],
    range: { from, to },
      ...
      warning:
        !extent?.last && state?.error
          ...
          : state?.error
            ...
            : kind === 'long_account_ratio'
              ? 'Bybit의 롱 포지션 보유 계정 수 / 전체 포지션 보유 계정 수입니다. 포지션 규모 비중과는 다릅니다.'
              : kind === 'open_interest'
                ? 'Bybit 한 거래소의 미결제 계약 양방향 합계입니다. 단위는 해당 코인 수량입니다.'
                : '펀딩비는 Bybit 한 거래소의 실제 정산 비율입니다.' +
                  (cursor !== null ? ' 과거 이력을 추가 수집하고 있습니다.' : ''),
    },
  };
}
