...
작업 버전 **0.15.0** · 운영 버전 **0.14.1**. 새 화면과 서버 기능은 로컬에 구현되어 있습니다. 2026-09-25 운영 D1 일일 쓰기 한도 소진으로 추가형 마이그레이션이 거절되어, 새 버전의 운영 배포는 보류했습니다. 유료 요금제를 추가하지 않았습니다. [구현·검증·배포 상태](docs/RELEASE15.md)
...
3. **지표 추가**에서 지표를 고르면 가격 아래 같은 시간축에 표시됩니다. 기본 두 개, 최대 여섯 개의 비교 지표를 지원합니다. 별도 단위는 각 축과 범례에 표시합니다.
4. **가격 위치 밴드**, 로그축, 기간을 변경하거나 지표를 눌러 산식·출처를 확인합니다. 차트 아래에서 CSV·이미지·URL 공유, 날짜별 표, 작업공간 저장을 엽니다.
5. **알림함**에서 확정 관측 신호와 일일 브리핑을 확인합니다. 읽음과 관심 규칙은 기기에 보관합니다. 새 서버 기능은 운영 마이그레이션·배포 후 활성화됩니다.
