...
2026-09-24 운영 검수: D1 무료 일일 쓰기 한도 초과로 서버의 새 관측 저장이 지연됐습니다. 화면은 마지막 정상 값과 지연 상태를 표시합니다. 코드·화면 배포와 자동 갱신 정상화는 별도이며, 상세 현황은 [0.14 운영 항목](docs/RELEASE14.md#운영-완료를-막는-실제-항목)을 확인하세요.
...
| 영역 | 구현 내용 |
|---|---|
| 탐색·화면 | 한글·영문 코인/지표 검색, 목적별 사이드바, 단일 코인 선택과 분석 탭, 접기·모바일 서랍, 다크/라이트 테마 |
...
| 브랜드 `/brand` | 웃는 시바견 PNG와 청록 워드마크, 사이트 아이콘·Open Graph 통일 |
| 리서치 `/research` | 37개 사용자 지정 X 계정 검색·즐겨찾기·코인 글 검색·미디어·영상 링크. 공개 RSS·영상·X 링크는 서버 자동 수집, 원천 비공개·차단 시 원문 연결 |
...
처음 접속하면 **선택 거래소의 전체 확보 이력**에서 긴 흐름을 먼저 봅니다. 상장 전 USD 참조가격은 별도 펼침 영역에서 확인합니다. 기간은 **1개월·3개월·6개월·올해·1년·3년·5년·전체**를 제공합니다. 직접 지정하거나 저장한 기간·URL은 존중하며, ‘전체’에서도 각 원천의 실제 확보 기간을 표시합니다.
