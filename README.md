...
현재 릴리스: **0.13.0**. BTC·DOGE·ETH 중심의 선물·코인별 도미넌스, 공식 출처가 붙은 역사·사건 차트, 공개 리서치 자동 수집을 연결했습니다. 핵심 세 코인의 서버 시세는 방문자 없이도 1분 주기로 갱신을 시도합니다. [변경·검증 기록](docs/UPGRADE13.md) · [리서치](https://coin-desk.pages.dev/research) · [역사](https://coin-desk.pages.dev/history?asset=BTC).
...
| 영역 | 구현 내용 |
|---|---|
| 탐색·화면 | 한글·영문 코인/지표 검색, 7개 목적지 사이드바, 단일 코인 선택과 분석 탭, 접기·모바일 서랍, 다크/라이트 테마 |
...
| 리서치 `/research` | 37개 사용자 지정 X 계정 검색·즐겨찾기·코인 글 검색·미디어·영상 링크. 자동 수집은 인증 및 활성화 설정 필요 |
