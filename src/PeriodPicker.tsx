    ...
    <>
      <div className="segments" aria-label="조회 기간">
        {PERIOD_OPTIONS.map((p) => (
          <button key={p.id} aria-pressed={value === p.id}
            className={value === p.id ? 'selected' : ''} onClick={() => onChange(p.id)}>
            {p.label}
          </button>
        ))}
      </div>
      <select className="period-mobile-select" aria-label="조회 기간" value={value}
