...
현재 기준은 [0.10 기록](UPGRADE10.md)입니다. 아래 0.8 이하 내용은 도입 당시 기록입니다.
...
- 화면 시세는 활성 탭에서 1분마다 확인합니다. 방문자가 없을 때 시세 수집 목표는 8분입니다. 온체인 일별 관측과 선물 정산 이력의 원천 주기는 별도입니다.
...
- 시세 요약은 화면이 보일 때 60초마다 조회합니다. 원천 갱신은 D1의 **60초 공유 잠금**을 확보한 요청만 수행합니다. 방문 중 **1분 갱신 목표**이며 원천 오류·재시도 대기에서는 마지막 정상 관측과 지연을 표시합니다. overview 응답은 추가 HTTP 캐시를 사용하지 않습니다. 방문자가 없을 때도 진행되는 8분 목표의 서버 수집과 별도입니다.
