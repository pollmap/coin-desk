...
## 0.10.0 운영 변경 · 2026-09-24

현재 기준은 [0.13 기록](UPGRADE13.md)과 이 문서 끝의 0.13 항목입니다. 아래 이전 버전 항목은 도입 당시 기록입니다.

- 선물 9개 작업은 매시간, 일별 선물 6개 작업은 6시간을 목표로 갱신합니다. 시간별 OI·롱 계정 비중은 최근 30일, 일별 시계열은 원천의 첫 관측까지 따로 확보합니다.
- `node scripts/bootstrap-derivatives.mjs`는 Bybit 원본과 재개 커서를 `work/derivatives-history.sqlite`에 보관하고 일별 6개 시계열만 `work/derivatives-history.sql`로 내보냅니다. 원격 적재 후 `node scripts/verify-derivatives.mjs`로 공개 API와 전량 대조합니다.
- Feed Worker는 3개 자산·5개 지표를 허용합니다. Feed의 서울 배치는 유지하고 메인 Worker의 강제 배치는 해제합니다.
- Feed는 Binance 현물 8개 자산의 시세·1시간/1일 봉(요청당 최대 32개), mempool.space의 현황·권장 수수료도 고정 경로로 조회합니다. 같은 공유 토큰을 요구하며 임의 URL이나 주문 요청은 받지 않습니다. 조회 결과는 메인 Worker에서 재검증합니다.
- Binance는 기존 공식 WebSocket API를 유지합니다. 시장데이터 전용 REST는 실제 Cloudflare 실행에서 403을 반환해 운영 기본값으로 채택하지 않았습니다.
- 시세 배치는 시장별 8개 자산씩 총 2개 작업입니다. 실패를 반복하는 시세·mempool 작업은 다른 지연 작업보다 무조건 앞서지 않습니다.
- 화면 시세는 활성 탭에서 1분마다 확인합니다. 방문자가 없을 때도 시세 수집 목표는 1분입니다. 온체인 일별 관측과 선물 정산 이력의 원천 주기는 별도입니다.
- 배포 전 오류의 재시도 대기만 해제할 수 있지만 실패·성공 기록은 덮어쓰지 않습니다. 정상화 여부는 실제 다음 수집의 성공으로 판단합니다.
...
- 매분 Cron은 방문자·개발 PC와 독립적으로 작업 하나를 선택합니다. 중복 실행은 분 단위 식별자와 120초 실행 잠금으로 막고, 중단된 실행은 다음 유효 시도에서 복구합니다. `cron_runs`는 최대 4,320개 슬롯(72시간)을 순환 보관하며 상태 API에는 최근 20개 실행과 48시간 집계를 보여줍니다.
- 현재가는 거래소별 배치로 묶습니다. BTC·DOGE·ETH는 **1분**, 보조 5개 코인은 **5분** 주기로 갱신을 시도합니다. 과거 이력과 별도 실행 경로이며 운영 지연 기준은 핵심 코인 **3분**, 보조 코인 **10분**입니다. 호출 제한·백오프·중단 시 정확한 최대 지연을 보장하지 않습니다.
...
- 시세 요약은 화면이 보일 때 60초마다 조회합니다. 원천 갱신은 D1의 **60초 공유 잠금**을 확보한 요청만 수행합니다. 방문 중 **1분 갱신 목표**이며 원천 오류·재시도 대기에서는 마지막 정상 관측과 지연을 표시합니다. overview 응답은 추가 HTTP 캐시를 사용하지 않습니다. 방문자가 없을 때도 진행되는 1분 목표의 서버 수집과 동일한 코인별 잠금을 공유합니다.
- 화면 시세의 실제 거래 시각이 5분을 넘으면 `liveStale` 또는 시세 지연 안내가 표시됩니다. 서버 운영은 3분, 화면 실제 거래 시각 표시는 5분 기준입니다. 거래가 없는 시장에서는 수집 성공과 실제 체결 시각을 구분합니다.
...
- 시간봉은 90일 목표로 작은 묶음씩 정리합니다. 기존 온체인 비활성 세대도 단계적으로 삭제합니다. 도미넌스 조회는 최초 관측부터 1,000개씩 커서로 반환하고 화면에서 전체 페이지를 결합합니다. 서비스 수집 전의 비중을 생성하지 않습니다.
...
- `worker/public-research.ts`는 별도 인증 없이 고정된 공개 출처 42개를 매분 1개씩 순환합니다. 정상 출처는 1시간 이후 갱신하며 실패 시 5분부터 지수 백오프로 최대 1시간까지 재시도 간격을 늘립니다. X 37계정의 공개 HTML, 공식 RSS 3개, 사용자 지정 영상 채널 Atom 2개입니다.
- `GET /api/v1/research/public`은 저장된 링크·제목·시각·명시된 코인 태그만 읽습니다. 요청 파라미터로 수집 URL을 받지 않고, 로그인·세션·비공개 API·유료 X API를 사용하지 않습니다. 링크는 HTTPS 및 출처별 호스트 허용 목록을 통과해야 합니다. 게시물·영상 전체 본문이나 이미지를 재게시하지 않습니다.
- HTML 차단·파싱 실패·빈 글은 `error`이며 정상으로 승격하지 않습니다. 최근 정상 메타데이터는 24시간 유지한 뒤 피드에서 제외합니다. 출처별 최대 15개, 전체 응답 최대 300개입니다. `ready`는 수집 성공이며 작성자의 신뢰성 인증이 아닙니다.
- 최초 채우기는 선택적으로 `node scripts/bootstrap_public.mjs` 및 `node scripts/bootstrap_derivatives.mjs`로 가능합니다. 산출 SQL은 `work/`에만 생성되고 자동 업로드되지 않습니다. 실제 원천을 파싱한 값만 생성하며 전자는 최신 서버 상태를 덮어쓰지 않고 후자는 기존 행을 보존합니다. 초기 채우기 이후 PC 프로세스는 필요하지 않습니다.
- 선물은 BTC/DOGE/ETH/SOL/XRP/LINK/ONDO/PEPE의 Bybit USDT 무기한 계약입니다. PEPE의 `1000PEPEUSDT` 미결제약정은 원천 값×1,000으로 PEPE 수량에 맞춥니다. 펀딩비·계정 비율에는 이 배수를 적용하지 않습니다. 선물 원천 어댑터 배포는 `npx wrangler deploy --config wrangler.feed.jsonc`입니다.
- 펀딩·일별 미결제약정·일별 계정비는 최초 페이지 이후 서버가 과거 페이지를 이어 수집합니다. 확보 중에는 차트 경고와 범위를 유지합니다. 1시간 이력은 원천·보관 범위 제한이 있습니다.
- 사건 연혁은 공식 자료를 검수한 편집 데이터입니다. 자동 수집 뉴스와 달리 사건 추가는 코드 검수를 거칩니다. 월만 확인된 사건은 날짜를 월로 표시하고 가격 인과관계로 단정하지 않습니다.
