...
웃는 시바견과 청록 워드마크를 사용합니다. [사용 중인 PNG](../public/brand/coin-desk-shiba-smile.png)는 기존 시바견을 image_gen으로 편집한 1254×1254 투명 이미지입니다. 황금색 털·크림색 볼·청록 스카프는 유지하고 웃는 눈과 입, 작은 분홍색 혀로 표정을 바꿨습니다. 사이트 로고·아이콘·공유 이미지가 동일한 파일을 사용합니다. DOGE 자산 식별 이미지는 별도 공식 이미지입니다.

사용한 편집 지시 요약: 기존 자세·색·윤곽을 유지하고 행복한 초승달 눈, 올라간 볼, 웃는 입과 작은 혀를 적용; 글자·코인·그래프를 추가하지 않고 투명 PNG로 생성. Image Gen 출력 원본은 보존하고 웹 자산으로 복사했습니다.

아래는 이전 버전의 디자인 기록입니다. 기존 다운로드 주소는 호환성을 위해 보존하며 현재 화면에서는 새 브랜드 이미지를 사용합니다.
